...
const EmailInbox: React.FC = () => {
  ...
  const getCategoryBadge = (labels: string[]) => {
    ...
    return (
      <div className="flex items-center gap-1.5">
        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border shadow-sm text-[10px] font-bold uppercase tracking-wider ${cls}`}>
          <Icon size={11} strokeWidth={2.5} />
          <span>{catName}</span>
        </div>
        {prioName && PRIORITY_CLS[prioName] && (
          <div className={`px-2 py-1 rounded-full border text-[10px] font-bold uppercase tracking-wider ${PRIORITY_CLS[prioName]}`}>
            {parts[1]}
          </div>
        )}
      </div>
  ...
  const EmailDetailPanel = ({ email, onClose }: { email: EmailMessage; onClose: () => void }) => (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-black text-slate-900 text-base leading-tight">{email.subject || '(No Subject)'}</h3>
              <p className="text-xs text-slate-400 font-medium mt-0.5">{email.from}</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="flex flex-wrap gap-2 mb-5">
            ...
            {email.labelIds.filter(l => ['INBOX','UNREAD','IMPORTANT'].includes(l)).map(l => (
              <span key={l} className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold uppercase tracking-wider">{l}</span>
            ))}
          </div>
          <div className="bg-slate-50 rounded-2xl p-4 text-sm text-slate-700 leading-relaxed font-medium min-h-[120px]">
            {email.snippet || 'No preview available.'}
            <span className="text-slate-400 italic"> [Full content in Gmail]</span>
          </div>
          <div className="mt-5 pt-4 border-t border-slate-100">
            <div className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">AI Classification</div>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={item.label} className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">{item.label}</div>
                  <div className="text-xs font-semibold text-slate-700 font-mono truncate">{item.value}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex gap-3">
          <a href={`https://mail.google.com/mail/#inbox/${email.threadId}`} target="_blank" rel="noopener noreferrer"
            className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition-colors shadow-lg shadow-blue-200">
            <Eye size={16} /> Open in Gmail
          </a>
          <button onClick={onClose} className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-100 transition-colors">
            Close
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC]">
      {/* Header */}
      <header className="sticky top-0 z-10 px-8 py-5 bg-white/80 backdrop-blur-md border-b border-slate-200/60 shadow-sm">
        <div className="max-w-5xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            {!isHumanReview && (
              <button onClick={handleBackfill} disabled={isBackfilling} className="group flex items-center gap-2.5 px-5 py-2.5 bg-slate-900 hover:bg-black text-white rounded-xl text-sm font-bold shadow-xl active:scale-95 transition-all disabled:opacity-50">
          ...
          {labelFilter && !isHumanReview && (
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-lg border border-blue-100 text-xs font-bold">
                ...
                <button onClick={() => window.history.pushState({},'','/#/inbox')} className="p-0.5 hover:bg-blue-100 rounded"><X size={11} /></button>
              </div>
            </div>
          )}
          ...
          {!isHumanReview && (
            <div className="relative max-w-2xl">
              ...
              <input type="text" placeholder="Find emails by subject..." value={searchTerm}
                ...
                className="w-full pl-11 pr-5 py-3.5 bg-slate-100 hover:bg-slate-200/50 focus:bg-white border-2 border-transparent focus:border-blue-500/20 rounded-2xl text-slate-700 font-medium shadow-inner outline-none transition-all text-sm" />
            </div>
          )}
        </div>
      </header>
        ...
        <div className="max-w-5xl mx-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center bg-white rounded-[32px] border-2 border-dashed border-slate-200">
              <div className="w-16 h-16 bg-slate-50 text-slate-200 rounded-3xl flex items-center justify-center mb-5">
                {isHumanReview ? <Activity size={32} /> : <Mail size={32} />}
              </div>
              <h3 className="text-xl font-bold text-slate-900 mb-2">{isHumanReview ? 'No Emails in Review Queue' : 'No Labeled Emails Yet'}</h3>
            ...
            <div className="grid gap-3">
              {filteredMessages.map((msg, idx) => (
                <div key={msg.id} onClick={() => setSelectedEmail(msg)}
                  ...
                  className="group relative p-5 bg-white hover:bg-slate-50 border border-slate-200/80 rounded-[24px] shadow-sm hover:shadow-lg hover:shadow-blue-500/5 hover:-translate-y-0.5 transition-all duration-200 cursor-pointer animate-in fade-in slide-in-from-bottom-3 flex items-center justify-between overflow-hidden">
                  <div className="flex items-center gap-5 flex-1 min-w-0">
                    <div className="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl flex items-center justify-center text-slate-400 group-hover:from-blue-600 group-hover:to-indigo-600 group-hover:text-white transition-all duration-300">
                      <Mail size={20} strokeWidth={1.5} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2.5 mb-1">
                        <h3 className="text-sm font-black text-slate-900 truncate group-hover:text-blue-600 transition-colors">{msg.subject || '(Empty Subject)'}</h3>
                      ...
                      {msg.snippet && <p className="mt-1.5 text-xs text-slate-400 truncate max-w-lg">{msg.snippet}</p>}
                    </div>
                  </div>
                  <div className="pl-4">
                    <div className="w-9 h-9 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-200 shadow-sm">
