    ...
    <div className="flex-1 overflow-y-auto bg-[#F8FAFC] dark:bg-slate-950">
      <div className="max-w-5xl mx-auto px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Pipeline Dashboard</h1>
            <p className="text-sm text-slate-400 dark:text-slate-500 font-medium mt-0.5">Last updated: {lastRefresh.toLocaleTimeString()}</p>
          </div>
          <button onClick={fetchStats} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm disabled:opacity-50">
            <RefreshCw size={15} className={loading ? 'animate-spin' : ''} /> Refresh
          </button>
        </div>

        {/* Status Bar */}
        <div className="flex items-center gap-3 mb-8 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-2">
            <div className={`w-2.5 h-2.5 rounded-full ${mcpOk ? 'bg-emerald-500 animate-pulse' : 'bg-red-400'}`} />
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">MCP Server</span>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${mcpOk ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'}`}>{mcpOk ? 'Online :9000' : 'Offline'}</span>
          </div>
          <div className="w-px h-5 bg-slate-200 dark:bg-slate-800" />
          <div className="flex items-center gap-2">
            <Cpu size={14} className="text-slate-400 dark:text-slate-500" />
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Groq LLM</span>
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400">llama-3.3-70b</span>
          </div>
          <div className="w-px h-5 bg-slate-200 dark:bg-slate-800" />
          <div className="flex items-center gap-2">
            <Zap size={14} className="text-slate-400 dark:text-slate-500" />
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">ACK Engine</span>
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400">Gemini 2.5</span>
          </div>
          <div className="w-px h-5 bg-slate-200 dark:bg-slate-800" />
          <div className="flex items-center gap-2">
            <Database size={14} className="text-slate-400 dark:text-slate-500" />
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Last Poll</span>
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{stats?.last_poll ?? '—'}</span>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-32">
            <div className="text-center">
              ...
              <p className="text-sm font-medium text-slate-400 dark:text-slate-500">Loading pipeline metrics...</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 mb-6">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="font-black text-slate-900 dark:text-slate-100 text-base">Category Breakdown</h2>
                <span className="ml-auto text-xs text-slate-400 dark:text-slate-500 font-medium">from labeled Gmail messages</span>
              </div>
                ...
                <div className="space-y-3">
                  {Object.entries(stats?.by_category ?? {}).sort(([,a],[,b]) => b - a).map(([cat, count]) => (
                    <div key={cat} className="flex items-center gap-3">
                      ...
                      <div className="w-8 text-right text-xs font-black text-slate-700 dark:text-slate-300">{count}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="font-black text-slate-900 dark:text-slate-100 text-base">Average Classification Confidence</h2>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-4xl font-black text-slate-900 dark:text-slate-100">{((stats?.avg_confidence ?? 0) * 100).toFixed(0)}%</div>
                <div className="flex-1">
                  ...
                  <div className="flex justify-between text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-1.5">
                    <span>0% — Random</span>
